...
test('loads spherical velocity values from v2 URL', async ({ page }) => {
  ...
  await page.goto(
    '/?v=2&vR=0.7&vPhi=1.5707963267948966&vTheta=0.7853981633974483',
  );

  const vGroup = page.getByRole('group', { name: 'boost velocity' });
  await expect(vGroup.getByRole('spinbutton', { name: 'r' })).toHaveValue(
    '0.7',
  );
...
test('loads boolean options from v2 URL', async ({ page }) => {
  ...
  await expect(fieldset.getByLabel('show components')).toBeChecked();
  await expect(fieldset.getByLabel('show poynting vector')).toBeChecked();
  await expect(fieldset.getByLabel('show lorentz force')).not.toBeChecked();
});

// ─── Legacy URL → state ───────────────────────────────────────────────────────

test('converts legacy (v1) spherical phi/theta to physics convention', async ({
  page,
}) => {
...
test('normalizes out-of-range theta in v2 URL', async ({ page }) => {
  ...
  const vGroup = page.getByRole('group', { name: 'boost velocity' });
  await expect(vGroup.getByRole('spinbutton', { name: 'θ' })).toHaveValue('45');
  await expect(vGroup.getByRole('spinbutton', { name: 'φ' })).toHaveValue(
    '225',
  );
});
...
test('upgrades legacy URL to v2 on load', async ({ page }) => {
  ...
  await page
    .getByRole('group', { name: 'original electric field' })
...
test('writes field values to URL on state change', async ({ page }) => {
  ...
  await page.waitForFunction(
    () => new URLSearchParams(window.location.search).get('eX') === '3',
  );
...
test('round-trips: state change → URL → reload → same state', async ({
  page,
}) => {
  const xInput = page
    .getByRole('group', { name: 'original electric field' })
  ...
  await page.waitForFunction(
    () => new URLSearchParams(window.location.search).get('eX') === '4',
  );
  const url = page.url();
